@testset "lbfgs" begin
  ...
  for FC in (Float64,)
    @testset "Data Type: $FC" begin
      ...
      # (x, stats) = lbfgs(A, b, reorthogonalization=true)
      # r = b - A * x
      # resid = norm(r) / norm(b)
      # @test(resid ≤ lbfgs_tol)
      # @test(stats.solved)
      ...
      # Expression: abs(q - qxs[end]) ≤ 1.0e-10
      # Evaluated: 0.015801137509880436 ≤ 1.0e-10
      @test stats.status == "on trust-region boundary"
